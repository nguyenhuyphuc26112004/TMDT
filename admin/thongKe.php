...
// --- 2. XỬ LÝ PHÂN TRANG CHO BẢNG ĐƠN HÀNG ---
...
// Doanh thu theo bộ lọc và Tổng số đơn để phân trang
$sqlFilterCount = "SELECT COUNT(*) as total_rows, SUM(tong_tien) as total_sum 
                   FROM don_hang 
                   WHERE trang_thai_thanh_toan = 'Đã thanh toán' 
                   AND ngay_dat BETWEEN '$startDate 00:00:00' AND '$endDate 23:59:59'";
$resFilterCount = mysqli_fetch_assoc(mysqli_query($con, $sqlFilterCount));
$totalFiltered = $resFilterCount['total_sum'] ?? 0;
$totalRows = $resFilterCount['total_rows'] ?? 0;
...
// Doanh thu 12 tháng
$months = []; $revenues = [];
$sqlMonthly = "SELECT m.m, COALESCE(SUM(d.tong_tien), 0) as rev
               ...
               LEFT JOIN don_hang d ON m.m = MONTH(d.ngay_dat) AND YEAR(d.ngay_dat) = YEAR(CURDATE()) AND d.trang_thai_thanh_toan = 'Đã thanh toán'
               GROUP BY m.m";
...
// Top 5 sản phẩm
$spNames = []; $spQtys = [];
$sqlTop = "SELECT sp.ten, SUM(ct.so_luong) as qty FROM ct_don_hang ct 
           ...
           GROUP BY sp.id ORDER BY qty DESC LIMIT 5";
...
<head>
    ...
    <style>
        ...
        /* GIỮ NGUYÊN CSS TRANG CHỦ (SIDEBAR) CỦA BẠN */
...
<div class="container">
    ...
    <div class="main">
        <h2><i class="fa-solid fa-chart-line"></i> Dashboard Thống Kê</h2>
        ...
        <div class="chart-box">
            <h4>Chi tiết đơn hàng thành công</h4>
            <table>
                <thead>
                    <tr>
                        <th>ID</th>
...
new Chart(document.getElementById('pieChart'), {
    ...
    data: {
        ...
        datasets: [{
            data: <?php echo json_encode($spQtys); ?>,
